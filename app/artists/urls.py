...
urlpatterns = [
    ...
    path(
        "artist-groups/<int:group_id>/members/add/", ArtistGroupMemberAddView.as_view(), name="artist-group-member-add"
    ),  # 기존 아티스트를 아티스트그룹에 추가
    path(
        "artist-groups/<int:group_id>/members/create/",
        ArtistGroupMemberCreateView.as_view(),
        name="artist-group-member-create",
    ),  # 아티스트그룹 생성시 아티스트들을 추가해서 생성
    path(
        "artist-groups/<int:group_id>/members/<int:artist_id>/delete/",
        ArtistGroupMemberDeleteView.as_view(),
        name="artist-group-member-delete",
    ),
