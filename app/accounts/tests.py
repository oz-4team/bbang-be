...
class UserModelTests(TestCase):
    def setUp(self):
        self.image_1 = UserImage.objects.create(  # 이미지 1 생성
            image_name="Image One", image_url="http://example.com/image1.jpg", image_type="jpg"
        )
        self.image_2 = UserImage.objects.create(  # 이미지 2 생성
            image_name="Image Two", image_url="http://example.com/image2.jpg", image_type="jpg"
        )

    def test_userimage_str(self):
        image_with_name = UserImage.objects.create(  # 이름있는 이미지 생성
            image_name="My Custom Image", image_url="http://example.com/custom.jpg", image_type="png"
        )
        self.assertEqual(str(image_with_name), "My Custom Image")

        image_without_name = UserImage.objects.create(  # 이름없는 이미지 생성
            image_name=None, image_url="http://example.com/default.jpg", image_type="jpg"
        )
